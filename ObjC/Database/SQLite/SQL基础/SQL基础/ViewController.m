...
- (void)fakeData {
    {
        ...
        NSError *error;
        if (![SQLString writeToFile:@"/Users/guoweilin/Desktop/shop.sqlite" atomically:YES encoding:NSUTF8StringEncoding error:&error]) {
            NSLog(@"error: %@", error);
        }
    }
...
- (void)connectDB {
    NSString *filePath = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"shop.sqlite"];
    int status = sqlite3_open(filePath.UTF8String, &_sqliteDB); // 如果数据库文件不存在, 系统会自动创建文件并初始化数据库.
    if (status == SQLITE_OK) {
        NSLog(@"数据库连接成功");
...
- (void)queryDB {
    ...
    if (status == SQLITE_OK) {
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            [self addShop:[NSString stringWithUTF8String:name]
...
- (IBAction)insert {
    ...
    [self addShop:self.nameField.text price:self.priceField.text];
    
    [self.tableView reloadData];
}

- (void)addShop:(NSString *)name price:(NSString *)price {
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    ...
    if (status == SQLITE_OK) {
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            const char *name = (const char *)sqlite3_column_text(stmt, 0);
            const char *price = (const char *)sqlite3_column_text(stmt, 1);
            [self addShop:[NSString stringWithUTF8String:name] price:[NSString stringWithUTF8String:price]];
        }
    }
